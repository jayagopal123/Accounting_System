...
function CreateBudgetPage() {
  ...
  const [lineItems, setLineItems] = useState([{ account: "", amount: "", notes: "" }]);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [fyRes, ccRes, acctRes] = await Promise.all([
          getFiscalYears(),
          getCostCenters(),
          getAccounts(),
        ]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (validLines.length === 0) {
        throw new Error("Add at least one budget line item with an account and amount.");
      }
    ...
    <MainLayout>
      <div className="page-card p-4">
        <div className="d-flex justify-content-between align-items-center mb-4">
          <div>
            <h5 className="page-header-title mb-1">Create Budget</h5>
            <p className="page-header-subtitle">Set budget allocations for a fiscal period</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="row g-3 mb-4">
            <div className="col-md-4">
              ...
              <input className="form-control" name="name" value={formData.name} onChange={handleChange} required placeholder="e.g. Annual Operating Budget" />
            </div>
            <div className="col-md-4">
            ...
            <div className="col-md-4">
              <label className="form-label">Cost Center (Optional)</label>
              <select className="form-select" name="costCenter" value={formData.costCenter} onChange={handleChange}>
                <option value="">None (Global)</option>
                {costCenters.map((cc) => (
                  <option key={cc._id} value={cc._id}>{cc.name} ({cc.code})</option>
                ))}
              </select>
            </div>
          ...
          <div className="d-flex gap-2 pt-3 border-top">
            <button className="btn btn-primary" disabled={submitting}>
              {submitting ? <><span className="spinner-border spinner-border-sm me-2" role="status"></span>Saving...</> : "Save Budget"}
            </button>
