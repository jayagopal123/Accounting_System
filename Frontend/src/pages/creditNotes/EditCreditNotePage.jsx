...
import { getCreditNoteById, updateCreditNote } from "../../services/creditNoteApi";
    ...
    try {
      ...
      setFormData({
        ...
        creditNoteDate: note.creditNoteDate
          ? new Date(note.creditNoteDate).toISOString().slice(0, 10)
    ...
    <MainLayout>
      <div className="page-card p-4">
        <div className="d-flex justify-content-between align-items-center mb-4">
          <div>
            <h5 className="page-header-title mb-1">Edit Credit Note</h5>
            <p className="page-header-subtitle">Modify credit note details</p>
          </div>
        ...
        {success && (
          <div className="alert alert-success d-flex align-items-center gap-2 mb-3" style={{ fontSize: "0.8rem", backgroundColor: "#ecfdf5", color: "#065f46", border: "none" }}>
            <span>Credit note updated successfully. Redirecting...</span>
          </div>
        )}
        {hasPermission("sales_invoices:update") && (
        <form onSubmit={handleSubmit}>
          ...
          <div className="d-flex gap-2 pt-3 border-top mt-3">
            <button className="btn btn-primary" disabled={submitting}>{submitting ? <><span className="spinner-border spinner-border-sm me-2" role="status"></span>Saving...</> : "Update Credit Note"}</button>
